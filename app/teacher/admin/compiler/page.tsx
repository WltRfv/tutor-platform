...
export default async function CompilerLogsPage() {
  ...
  const stats = {
    ...
    suspicious: runs.filter((r) => (r.aiSuspicion || 0) >= 60).length,
  };
    ...
    <div className="p-8 max-w-6xl">
      <div className="mb-8 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Последние 100 запусков + детектор ИИ
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <div className="backdrop-blur-xl bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
          <div className="text-xs text-amber-300 mb-1 flex items-center gap-1">
            <AlertTriangle className="h-3 w-3" /> Подозрительных
          </div>
      ...
      {runs.length === 0 ? (
        ...
        <div className="space-y-3">
          {runs.map((r) => {
            const suspicion = r.aiSuspicion || 0;
            const isSuspicious = suspicion >= 60;
            const isMedium = suspicion >= 30 && suspicion < 60;
                  ...
                  <div className="mb-3 flex flex-wrap gap-3 text-xs">
                    ...
                    <span className="text-slate-400">
                      ...
                      <span
                        className={cn(
                          'font-medium',
                          (r.pastedChars || 0) > (r.typedChars || 0)
                            ? 'text-amber-400'
                            : 'text-slate-200'
                        )}
                      >
                    ...
                    <span className="text-slate-400">
                      Скорость:{' '}
                      <span className="text-slate-200 font-medium">
                        {r.avgTypingSpeed} симв/с
                      </span>
                    </span>
                    <span className="text-slate-400">
                      Вставок: <span className="text-slate-200">{r.pasteCount}</span>
                    </span>
                  </div>
                )}
